package nineTailed.actions;

import com.megacrit.cardcrawl.actions.AbstractGameAction;
import com.megacrit.cardcrawl.actions.common.ApplyPowerAction;
import com.megacrit.cardcrawl.cards.blue.Barrage;
import com.megacrit.cardcrawl.cards.red.Intimidate;
import com.megacrit.cardcrawl.cards.red.Shockwave;
import com.megacrit.cardcrawl.characters.AbstractPlayer;
import com.megacrit.cardcrawl.dungeons.AbstractDungeon;
import com.megacrit.cardcrawl.monsters.AbstractMonster;
import com.megacrit.cardcrawl.orbs.AbstractOrb;
import com.megacrit.cardcrawl.powers.VulnerablePower;
import com.megacrit.cardcrawl.powers.WeakPower;
import com.megacrit.cardcrawl.powers.watcher.WaveOfTheHandPower;
import nineTailed.orbs.Clone;

public class HaremAction extends AbstractGameAction {
    private int multiplier;
    AbstractPlayer p;

    public HaremAction(int multiplier) {
        this.multiplier = multiplier;
        p = AbstractDungeon.player;
    }

    public void update() {
        int count = 0;
        for (AbstractOrb o : p.orbs) {
            if (o instanceof Clone) {
                count++;
            }
        }
        int amt = count * multiplier;
        for (AbstractMonster m : AbstractDungeon.getCurrRoom().monsters.monsters) {
            this.addToBot(new ApplyPowerAction(m, p, new WeakPower(m, amt, false), amt, true));
            this.addToBot(new ApplyPowerAction(m, p, new VulnerablePower(m, amt, false), amt, true));
        }
        this.isDone = true;
    }
}
